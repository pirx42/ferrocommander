...
pub(crate) fn dispatch(shell: &Rc<RefCell<Shell>>, action: Action) {
    ...
    match action {
        ...
        Action::CursorPageUp => shell.borrow_mut().active_pane().move_by_page(-1),
        Action::CursorPageDown => shell.borrow_mut().active_pane().move_by_page(1),
        Action::Activate => {
        ...
        Action::ExtendMarkPageUp => shell.borrow_mut().active_pane().extend_mark_by_page(-1),
        Action::ExtendMarkPageDown => shell.borrow_mut().active_pane().extend_mark_by_page(1),
...
pub(crate) fn spawn_command(shell: &Rc<RefCell<Shell>>, directory: VfsPath, line: String) {
    let receiver = fc_core::command::spawn(directory, line);
    let shell = shell.clone();
...
pub(crate) fn go_to_drive(shell: &Rc<RefCell<Shell>>, target: usize, mount: &VfsPath) {
    ...
    if let Some(from) = fc_core::vfs::mount_for(&leaving, &fc_core::vfs::mount_points()) {
        state
            .drives
...
pub(crate) fn start_drive_selection(shell: &Rc<RefCell<Shell>>, target: usize) {
    ...
    let places: Vec<(String, String)> = fc_core::vfs::mount_points()
        .into_iter()
...
pub(crate) fn start_search(shell: &Rc<RefCell<Shell>>) {
    ...
    dialogs::Search::open(
        ...
        move |criteria, cancel| {
            fc_core::search::spawn(Arc::clone(&searching), root.clone(), criteria, cancel)
        },
        move |path| {
...
pub(crate) fn start_viewing(shell: &Rc<RefCell<Shell>>) {
    ...
    let Ok(view) = fc_core::viewer::View::open(fs.as_ref(), path) else {
        return;
    };
...
pub(crate) fn start_compare(shell: &Rc<RefCell<Shell>>) {
    ...
    if let Some(tool) = tool {
        ...
        fc_core::command::run_with_paths(&tool, &left, &right);
        return;
    }
    ...
    let receiver = fc_core::compare::spawn(left_fs, left, right_fs, right);
    glib::spawn_future_local(async move {
        let Ok(answer) = receiver.recv().await else {
            return;
        };
        match answer {
            Ok(fc_core::compare::Comparison::Rows(rows)) => {
                dialogs::open_compare(&window, &title, &rows);
            }
            Ok(fc_core::compare::Comparison::Verdict(verdict)) => {
                let text = match verdict {
                    fc_core::compare::Verdict::Identical => COMPARE_IDENTICAL.to_string(),
                    fc_core::compare::Verdict::Differ { first_difference } => {
...
pub(crate) fn start_editing(shell: &Rc<RefCell<Shell>>) {
    ...
    fc_core::command::open_with(&editor, &path);
}
        ...
        move |name| {
            ...
            submit_then(
                ...
                move || {
                    ...
                    fc_core::command::open_with(&editor, &target);
                },
            );
        },
    );
