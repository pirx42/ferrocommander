...
//! directory contains and how it is ordered belongs to `fc-core`.
...
use fc_core::config;
use fc_core::ops::{Destination, Job};
...
fn build_window(app: &gtk::Application) {
    ...
    let backend: Arc<dyn fc_core::vfs::VirtualFs> = Arc::new(LocalFs);
...
fn fill_drive_bar(bar: &gtk::Box, shell: &Rc<RefCell<Shell>>) {
    for mount in fc_core::vfs::mount_points() {
