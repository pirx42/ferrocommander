...
use fc_core::archive::{format_for, ArchiveFs, Format};
use fc_core::ops::{
    self, Answer, CancelToken, DeleteMode, Destination, Job, Progress, Report, Resolution, Silent,
};
use fc_core::vfs::{Attributes, Entry, EntryKind, LocalFs, Store, VfsError, VfsPath, VirtualFs};
    ...
    for name in ["out.zip", "out.tar"] {
        ...
        LocalFs
            .set_attributes(
                ...
                fc_core::vfs::attributes_from_unix_mode(0o755),
            )
            .unwrap();
        ...
        assert_eq!(
            fc_core::vfs::render_attributes(packed.attributes),
            fc_core::vfs::render_attributes(fc_core::vfs::attributes_from_unix_mode(0o755)),
            "{name} lost the mode"
        );
    }
}
