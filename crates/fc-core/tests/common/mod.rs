...
use fc_core::ops::{Answer, Conflict, ConflictResolver};
use fc_core::vfs::{LocalFs, VfsPath, VirtualFs};
...
macro_rules! delegate_vfs {
    ($target:ty) => {
        impl fc_core::vfs::VirtualFs for $target {
            ...
            fn store(&self) -> fc_core::vfs::Store {
                self.inner.store()
            }
            fn read_dir(
                &self,
                path: &fc_core::vfs::VfsPath,
            ) -> Result<Vec<fc_core::vfs::Entry>, fc_core::vfs::VfsError> {
                self.read_dir_impl(path)
            }
            fn stat(
                &self,
                path: &fc_core::vfs::VfsPath,
            ) -> Result<fc_core::vfs::Entry, fc_core::vfs::VfsError> {
                self.inner.stat(path)
            }
            fn create_dir(
                &self,
                path: &fc_core::vfs::VfsPath,
            ) -> Result<(), fc_core::vfs::VfsError> {
            ...
            fn rename(
                &self,
                from: &fc_core::vfs::VfsPath,
                to: &fc_core::vfs::VfsPath,
            ) -> Result<(), fc_core::vfs::VfsError> {
            ...
            fn open_read(
                &self,
                path: &fc_core::vfs::VfsPath,
            ) -> Result<Box<dyn std::io::Read + Send>, fc_core::vfs::VfsError> {
            ...
            fn read_at(
                ...
                offset: u64,
                len: usize,
            ) -> Result<Vec<u8>, fc_core::vfs::VfsError> {
                self.inner.read_at(path, offset, len)
            }
            fn create_file(
                &self,
                path: &fc_core::vfs::VfsPath,
            ) -> Result<Box<dyn std::io::Write + Send>, fc_core::vfs::VfsError> {
            ...
            fn set_attributes(
                &self,
                path: &fc_core::vfs::VfsPath,
                attributes: fc_core::vfs::Attributes,
            ) -> Result<(), fc_core::vfs::VfsError> {
                self.inner.set_attributes(path, attributes)
            }
            fn trash(&self, path: &fc_core::vfs::VfsPath) -> Result<(), fc_core::vfs::VfsError> {
...
impl VirtualFs for Rooted {
    ...
    fn read_dir(&self, path: &VfsPath) -> Result<Vec<fc_core::vfs::Entry>, fc_core::vfs::VfsError> {
        LocalFs.read_dir(&self.at(path))
    }
    fn stat(&self, path: &VfsPath) -> Result<fc_core::vfs::Entry, fc_core::vfs::VfsError> {
        LocalFs.stat(&self.at(path))
    }
    fn create_dir(&self, path: &VfsPath) -> Result<(), fc_core::vfs::VfsError> {
        LocalFs.create_dir(&self.at(path))
    }
    fn remove_dir(&self, path: &VfsPath) -> Result<(), fc_core::vfs::VfsError> {
        LocalFs.remove_dir(&self.at(path))
    }
    fn remove_file(&self, path: &VfsPath) -> Result<(), fc_core::vfs::VfsError> {
        LocalFs.remove_file(&self.at(path))
    }
    fn rename(&self, from: &VfsPath, to: &VfsPath) -> Result<(), fc_core::vfs::VfsError> {
    ...
    fn trash(&self, path: &VfsPath) -> Result<(), fc_core::vfs::VfsError> {
