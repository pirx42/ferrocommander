...
use tc_core::search::{spawn, Criteria, Results, CONTENT_WINDOW};
use tc_core::vfs::{LocalFs, VfsError, VfsPath, VirtualFs};
...
impl Refusing {
    fn read_dir_impl(&self, path: &VfsPath) -> Result<Vec<tc_core::vfs::Entry>, VfsError> {
        if *path == self.refuse {
