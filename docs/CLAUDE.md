...
- [packaging.md](packaging.md) — the Ubuntu `.deb`, the Windows zip and the
  macOS `.app`: what each holds, how their version ties to the title bar, why
  the runtime dependencies of all three are derived rather than listed, and
  why the logic is in scripts rather than in the workflow.
...
- [plans/](plans/archive/) — plan documents. A plan is written before the work,
  ...
  | [compare by content](plans/archive/2026-08-31-compare-by-content.md) | Total Commander's Compare by Content: `Ctrl+Shift+C`, the marked-pair/namesake/cursor cascade, a side-by-side view with intra-line marks whose two sides cannot drift because they share one scroller, and the `compare_tool` line that replaces it all — plus the keymap gate's first *code* catch, and the byte↔char round trip the audit deleted |
  | [drop the tc prefix](plans/archive/2026-09-01-drop-the-tc-prefix.md) | the crates became `fc-core` and `fc-app`: 441 substitutions over 98 files, the import blocks rustfmt had to reorder because the new name sorts where the old one did not, and a `check-naming.py` in the gate so nothing brings the spelling back — which failed on its own documentation before the line moved to the name prefix |
  | [quick view](plans/archive/2026-09-01-quick-view.md) | Total Commander's `Ctrl+Q`: the other pane becomes a window onto whatever the cursor is on, following it — quit moved to `Alt+F4` and the harness line eleven closing tests lean on with it, a directory summarised by a walk that is asked for and never waited for, and the benchmark that flattered itself by 25× until it was pointed at a hard case |
  | [testing round fixes](plans/archive/2026-09-01-testing-round-fixes.md) | five findings from a person using the program: a pane that scrolled when nothing asked it to, a flicker one frame long, a Windows drive root that landed in the application's own folder, a missing free-space figure, and a backgrounded job with nothing on screen — plus the `Space` diagnosis this plan got wrong three times before instrumenting the adjustment settled it |
  | [testing round two](plans/archive/2026-09-01-second-testing-round.md) | seven more items from using the program: a viewport that moved when nothing asked it to — three reports, one cause, and four wrong fixes before the frame log named it — folder sizes that changed every time they were asked for, and the row Total Commander draws: an icon, brackets, and half the height |
  | [testing round three](plans/archive/2026-09-01-third-testing-round.md) | the two viewers and `Enter`: a page key that jumped to the top because the offset clamped to zero and the widget had no notion of the screen, a diff separator given a third of the window by one word, line numbers, and the `/bin/sh` line that had `Enter`, `F4`, the compare tool and the command line all doing nothing on Windows |
